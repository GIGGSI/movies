...
async function register(username, password) {
    return (await fetch(host(endpoinsts.REGISTER), {
        method: 'POST',
        headers: {
...
function logout() {
    ...
    return fetch(host(endpoinsts.LOGOUT), {
        headers: {
            'user-token': token
        }
    });
...
async function getMovies() {
    ...
    return (await fetch(host(endpoinsts.MOVIES), {
        headers: {
            'user-token': token
        }
    })).json();
...
async function getMovieById(id) {
    ...
    return (await fetch(host(endpoinsts.MOVIE_BY_ID + id), {
        headers: {
            'user-token': token
        }
    })).json();
...
async function getMoviesByOwner(ownerId) {
    ...
    return (await fetch(host(endpoinsts.MOVIES + `?where=ownerId%3D%27${ownerId}%27`), {
        headers: {
            'Content-Type': 'application/json',
            'user-token': token
        }
